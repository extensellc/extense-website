...
import sys, os, argparse, html, http.server, socketserver, threading, webbrowser, re
...
<body>
  <header class="page-header">
    <div>
      <strong>DITA Preview</strong> &nbsp;·&nbsp; {html.escape(fname)}
    </div>
...
def main():
    ...
    try:
        # Parse with DTD recovery — strip DOCTYPE to avoid resolution issues
        with open(filepath, "r", encoding="utf-8") as f:
            content = f.read()
        # Remove DOCTYPE declaration to avoid DTD resolution errors
        content = re.sub(r'<!DOCTYPE[^>]*>', '', content, count=1)
        root = ET.fromstring(content)
    except ET.ParseError as e:
        print(f"Error: XML parse error in {filepath}: {e}", file=sys.stderr)
        sys.exit(1)
